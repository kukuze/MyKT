        ...
        for user in users:
            ...
            for i, item in enumerate(seq):
                ...
                if torch.isnan(loss):
                    ...
                    nan_records.append(nan_info)

                    # 记录到日志文件
                    logging.error(f'NaN detected - Epoch: {epoch + 1}, User: {user}, Position: {i}')

                    # 保存异常记录到文件
                    save_path = os.path.join(Constants.MODEL_SNAPSHOT_PATH, f'nan_records_epoch_{epoch + 1}.pkl')
                    torch.save(nan_records, save_path)
